...
export function useGhostPay() {
  ...
  const connect = async () => {
    if (window.ethereum) {
      ...
      try {
        ...
        // Force Arbitrum Sepolia (chainId 421614)
  ...
  const refreshBalance = useCallback(async () => {
    if (contract && account) {
      try {
        ...
        console.warn('Balance unavailable — Nox may not support this network');
        setBalance('N/A');
      }
    }
  }, [contract, account]);
  ...
  const distributePayroll = async (employees: string[], amounts: string[]) => {
    ...
    try {
      ...
      const nox = await createEthersHandleClient(signer);

      const handles = [];
      let inputProof = '0x';

      for (let i = 0; i < amounts.length; i++) {
        const parsedAmount = ethers.parseUnits(amounts[i], 18);
        const { handle, proof } = await nox.encrypt(parsedAmount);
        handles.push(handle);
        inputProof = proof;
      }
  ...
  return {
    ...
    wrapFunds: async (amount: string) => {
      ...
      try {
        ...
        const appTx = await underlyingContract.approve(
          GHOST_PAY_ADDRESS,
          parsedAmount,
        );
        await appTx.wait();
